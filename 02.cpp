...
int main()
{
    ...
    while(completed!=number_of_process){
        // cout<<current_time<<" "<<ready_queue.size()<<endl;
        if(!ready_queue.empty()){
            int cur = ready_queue.top().index;
            int bt = ready_queue.top().burst_time;
            ready_queue.pop();
            if(gantt_chart.empty() or gantt_chart.back().first!=allProcess[cur].process_id){
                gantt_chart.push_back({allProcess[cur].process_id,current_time});
            }
            // else{
            //     gantt_chart.back().second=current_time;
            // }
            bt--;
            current_time++;
            if(bt==0){
